...
function grad_weibull(σ::Matrix{Float64}, λ::Vector{Float64}, w::Float64, xd::Float64, xg::Float64)
  ...
  θ = w*exp(λ[1]+σ_γ)
  α = exp(λ[2])
  ∂₁g = (exp(-(xd/θ)^α)*α*(xd/θ)^α/θ-exp(-(xg/θ)^α)*α*(xg/θ)^α/θ)*w*exp(λ[1]+σ_γ)
  ∂₂g = (-exp(-(xd/θ)^α)*(xd/θ)^α*log(xd/θ)+exp(-(xg/θ)^α)*(xg/θ)^α*log(xg/θ))*exp(λ[2])
  ∂ₛg = ∂₁g .* σ' 
  [∂₁g; ∂₂g; 0; ∂ₛg]
end
...
function pdf_frailty(bio::Biotope, i::Int64, selVar::Vector{Int64})
  function proxyDistCov(w::Float64, λ::Vector{Float64})
    return prod_weibull(bio,i, selVar,w,λ) * Γh(exp(λ[3]), w)
  end
  return proxyDistCov
end
...
@btime  Γh(3.4,5.2) -> 25.503 ns (0 allocations: 0 bytes)
@btime  pdf(Gamma(3.4,1. / 3.4), 5.2) ->  235.772 ns (2 allocations: 48 bytes)
"""
